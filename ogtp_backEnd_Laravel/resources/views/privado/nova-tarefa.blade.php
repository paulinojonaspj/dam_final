        ...
        <div class="authentication-inner">
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    @if (isset($update))
                        <h4 class="mb-2"> <a href="{{ url('/tarefas',$atividade_id) }}"><span>Voltar</span> | </a> Actualizar Tarefa 👋</h4>
                    @else
                        <h4 class="mb-2"><a href="{{ url('/tarefas',$atividade_id) }}"><span>Voltar</span> | </a> Nova Tarefa 👋</h4>
                    @endif

                    <form method="post" action="{{ url('nova-tarefa') }}" class="mb-3">
                        @csrf
                        ...
                        <div class="mb-3">
                            ...
                            <select required class="form-control" name="estado" autocomplete="off">
                                ...
                                <option
                                    {{ Session::get('estado', $dados->estado ?? '') === 'Em curso' ? 'Selected' : '' }}
                                    value="Em curso">Em
                                    curso</option>
                        ...
                        <div class="mb-3">
                            ...
                            <input value="{{ Session::get('hora_inicio', $dados->hora_inicio ?? '') }}" required
                                type="time" class="form-control" name="hora_inicio" autocomplete="off" />
                        </div>

                        <div class="mb-3">
                            <label for="email" class="form-label">Data fim</label>
                            <input required value="{{ Session::get('data_fim', $dados->data_fim ?? '') }}"
                                type="date" class="form-control" name="data_fim" autocomplete="off" />
                        </div>

                        <div class="mb-3">
                            <label class="form-label">Hora fim</label>
                            <input value="{{ Session::get('hora_fim', $dados->hora_inicio ?? '') }}" required
                                type="time" class="form-control" name="hora_fim" autocomplete="off" />
                        </div>


                        <input hidden value="{{ $update ?? 0 }}" name="update" id="idA" />
                        <input hidden value="{{ $atividade_id ?? 0 }}" name="atividade_id" />
